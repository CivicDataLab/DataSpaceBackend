...
class MockResourceChartDetails:
    ...
    def __init__(self, chart_type="GROUPED_BAR_VERTICAL", title="Test Chart", description="Test Description", options=None):
...
class TestGroupedBarChart(unittest.TestCase):
    def setUp(self):
        ...
        self.test_data = pd.DataFrame({
            ...
            'time': ['2023-01', '2023-01', '2023-02', '2023-02', '2023-03', '2023-03']
        })
    ...
    def test_vertical_grouped_bar_chart(self):
        ...
        chart_details = MockResourceChartDetails(chart_type="GROUPED_BAR_VERTICAL", options=options)
        chart = GroupedBarChart(chart_details, self.test_data)
    ...
    def test_horizontal_grouped_bar_chart(self):
        ...
        chart_details = MockResourceChartDetails(chart_type="GROUPED_BAR_HORIZONTAL", options=options)
        chart = GroupedBarChart(chart_details, self.test_data)
        result = chart.create_chart()
    ...
    def test_value_aggregation(self):
        ...
        options = {
            'x_axis_column': self.x_field,
            'y_axis_column': {'field': self.y_field, 'label': 'Value', 'aggregate_type': 'AVERAGE'},
            'group_column': self.group_field
        }
        chart_details = MockResourceChartDetails(chart_type="GROUPED_BAR_VERTICAL", options=options)
        chart = GroupedBarChart(chart_details, test_data)
        result = chart.create_chart()
    ...
    def test_time_based_grouped_bar_chart(self):
        ...
        self.assertTrue(all(period in x_axis.get('data') for period in ['2023-01', '2023-02', '2023-03']))
    ...
    def test_value_mapping(self):
        ...
        options = {
            'x_axis_column': self.x_field,
            'y_axis_column': {'field': self.y_field, 'label': 'Value', 'value_map': value_map},
            'group_column': self.group_field
        }
        chart_details = MockResourceChartDetails(chart_type="GROUPED_BAR_VERTICAL", options=options)
        chart = GroupedBarChart(chart_details, test_data)
