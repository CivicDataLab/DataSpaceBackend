...
class MockResourceChartDetails:
    ...
    def __init__(self, chart_type="BAR_VERTICAL", title="Test Chart", description="Test Description", options=None):
...
class TestBarChart(unittest.TestCase):
    ...
    def test_vertical_bar_chart(self):
        ...
        chart_details = MockResourceChartDetails(chart_type="BAR_VERTICAL", options=options)
        chart = BarChart(chart_details, self.test_data)
    ...
    def test_horizontal_bar_chart(self):
        ...
        chart_details = MockResourceChartDetails(chart_type="BAR_HORIZONTAL", options=options)
        chart = BarChart(chart_details, self.test_data)
        result = chart.create_chart()
    ...
    def test_bar_styling(self):
        ...
        options = {
            'x_axis_column': self.x_field,
            'y_axis_column': [{'field': self.y_field, 'label': 'Value', 'color': '#FF0000'}]
        }
        chart_details = MockResourceChartDetails(chart_type="BAR_VERTICAL", options=options)
        chart = BarChart(chart_details, self.test_data)
        result = chart.create_chart()
    ...
    def test_value_aggregation(self):
        ...
        options = {
            'x_axis_column': self.x_field,
            'y_axis_column': [{'field': self.y_field, 'label': 'Value', 'aggregate_type': 'SUM'}]
        }
        chart_details = MockResourceChartDetails(chart_type="BAR_VERTICAL", options=options)
        chart = BarChart(chart_details, self.test_data)
        result = chart.create_chart()
