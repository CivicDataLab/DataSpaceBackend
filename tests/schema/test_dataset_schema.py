...
from api.schema.dataset_schema import _add_update_dataset_metadata, DSMetadataItemType

class TestAddUpdateDatasetMetadata(unittest.TestCase):
    def setUp(self):
        self.dataset_id = uuid.uuid4()
        self.dataset = Mock(spec=Dataset, id=self.dataset_id)

    @patch("api.schema.dataset_schema.Dataset.objects.create")
    @patch("api.schema.dataset_schema.DatasetMetadata.objects.get")
    def test_add_new_metadata(self, mock_metadata_get, mock_dataset_create):
        ...
        with patch("api.schema.dataset_schema._delete_existing_metadata") as mock_delete:
    ...
    def test_update_existing_metadata(self, mock_metadata_get, mock_dataset_create):
        mock_dataset_create.return_value = self.dataset
        mock_metadata_get.return_value = Mock(spec=Metadata, enabled=True)
        ...
        self.assertEqual(
            self.dataset.datasetmetadata_set.get(metadata_item_id="1").value, "Updated Value 1"
        )
        self.assertEqual(
            self.dataset.datasetmetadata_set.get(metadata_item_id="2").value, "Updated Value 2"
        )

    @patch("api.schema.dataset_schema.Dataset.objects.create")
    @patch("api.schema.dataset_schema.Metadata.objects.get")
    def test_delete_existing_metadata(self, mock_metadata_get, mock_dataset_create):
        mock_dataset_create.return_value = self.dataset
        mock_metadata_get.return_value = Mock(spec=Metadata, enabled=True)
    ...
    @patch("api.schema.dataset_schema.Dataset.objects.create")
    @patch("api.schema.dataset_schema.Metadata.objects.get")
    def test_handle_non_existent_metadata_id(self, mock_metadata_get, mock_dataset_create):
        mock_dataset_create.return_value = self.dataset
    ...
    @patch("api.schema.dataset_schema.Dataset.objects.create")
    @patch("api.schema.dataset_schema.Metadata.objects.get")
    def test_handle_disabled_metadata_field(self, mock_metadata_get, mock_dataset_create):
        mock_dataset_create.return_value = self.dataset
        ...
        self.assertEqual(
            str(context.exception), "Metadata with ID 1 is not enabled."
        )
