...
A repo (optionally a branch and sub-folder) is treated as a dataset. Two
requests per import: the repo metadata and the README. Public repos need no
...
class GitHubImporter(PlatformImporter):
    ...
    def fetch_dataset_info(self, identifier: str) -> PlatformDatasetInfo:
        ...
        description = self._description(full_name, headers) or str(meta.get("description") or "")
    ...
    # -- pieces -------------------------------------------------------------- #
    def _description(self, full_name: str, headers: Dict[str, str]) -> str:
        try:
            response = self._get(
