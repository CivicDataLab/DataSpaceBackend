...
class BaseChart(ABC):
    def __init__(self, chart_details: ResourceChartDetails, data: pd.DataFrame) -> None:
        self.chart_details = chart_details
        self.data = data
        # options will contain Django model objects after decoding
        self.options: Dict[str, Union[DjangoFieldLike, Dict[str, Any], List, Any]] = (
            chart_details.options
        )

    def get_chart_class(self) -> Any:
        return CHART_TYPE_MAP.get(self.chart_details.chart_type)

    def _process_value(self, value: str, operator: str) -> Union[str, List[str]]:
        """Process the filter value based on the operator."""
        if operator in ("in", "not in"):
            return (
                [val.strip() for val in value.split(",")] if "," in value else [value]
            )
        return value
    ...
    def filter_data(self) -> pd.DataFrame:
        ...
        """
        filtered_data: pd.DataFrame = self.data
        ...
        conditions = []
        for filter_condition in cast(List[ChartFilter], self.chart_details.filters):
        ...
        return (  # type: ignore[no-any-return]
            filtered_data[pd.concat(conditions, axis=1).all(axis=1)]
            if conditions
            else filtered_data
        )
    ...
    def get_y_axis_bounds(self, filtered_data: pd.DataFrame) -> tuple:
        ...
        for y_axis_column in self._get_y_axis_columns():
            y_axis_field = cast(DjangoFieldLike, y_axis_column["field"])
            column_name = y_axis_field.field_name
            if column_name in filtered_data.columns:
                y_values.extend(
                    filtered_data[column_name].dropna().astype(float).tolist()
                )
    ...
    def initialize_chart(self, filtered_data: Optional[pd.DataFrame] = None) -> Chart:
        ...
        chart = self.get_chart_class()(init_opts=self.get_init_opts())
    ...
    def _handle_time_based_data(
        self, chart: Chart, filtered_data: pd.DataFrame, time_column: DjangoFieldLike
    ) -> None:
        """Handle time-based data with aggregation."""
        # Group data by time periods
        time_groups = filtered_data.groupby(time_column.field_name)
        selected_groups = cast(List[str], self.options.get("time_groups", []))
        ...
        if x_field == time_column.field_name:
            time_axis_data = sorted(
                [
                    str(time)
                    for time in time_groups.groups.keys()
                    if str(time) in selected_groups
                ]
            )
            chart.add_xaxis(time_axis_data)
            ...
            for y_axis_column in self._get_y_axis_columns():
                ...
                aggregate_type = str(y_axis_column.get("aggregate_type", ""))

                y_values = []
                for time_val in time_axis_data:
                    period_data = time_groups.get_group(time_val)
                    value = self._apply_aggregation(
                        period_data, field_name, aggregate_type
                    )
            ...
            all_x_values.sort()  # type: ignore[attr-defined]

            # Create x-axis labels with time periods
            combined_axis_data: List[str] = []
            for x_val in all_x_values:
                x_str = str(x_val)
                for time_val_hash in time_groups.groups.keys():
                    time_str = str(time_val_hash)
                    if time_str in selected_groups:
                        combined_axis_data.append(f"{x_str} ({time_str})")
            chart.add_xaxis(combined_axis_data)
            ...
            for y_axis_column in self._get_y_axis_columns():
                ...
                for x_val in all_x_values:
                    for time_val_hash in time_groups.groups.keys():
                        time_str = str(time_val_hash)
                        if time_str not in selected_groups:
                            continue

                        period_data = time_groups.get_group(time_val_hash)
                        # Filter data for current x value
                        x_filtered_data = period_data[period_data[x_field] == x_val]
                        value = self._apply_aggregation(
                            x_filtered_data, field_name, aggregate_type
                        )
                        y_values.append(value)
    ...
    def _get_series_name(self, y_axis_column: Dict[str, Any]) -> str:
        ...
        if isinstance(y_axis_column, dict):
            ...
            return y_axis_column.get("label") or field.field_name
        field = cast(DjangoFieldLike, y_axis_column)
